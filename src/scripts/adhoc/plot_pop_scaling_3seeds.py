"""Plot GSM8K population scaling averaged across seeds 42, 43, 44.

Seed 42 N=1/2/4 are stitched from consecutive [1st]+[2nd] runs and end short;
they contribute NaN beyond their last point when averaging.

Usage:
  uv run python -m src.scripts.plot_pop_scaling_3seeds
  uv run python -m src.scripts.plot_pop_scaling_3seeds --out results/pop_scaling_3seeds.png
"""
...
# Each entry is a list of log paths to stitch consecutively for that seed.
# Paths are relative to repo root.
RUNS: dict[int, list[list[str]]] = {
    1: [
        # seed 42: [1st] then [2nd] (2nd continues train_fwd naturally)
        [
    ...
    2: [
        [
            "results/seed42_pop_scale/[1st] gsm8k_n2_20260413_150355/log.jsonl",
            "results/seed42_pop_scale/[2nd] gsm8k_n2_20260414_010958/log.jsonl",
        ],
        ["results/seed43_pop_scale/gsm8k_n2_s2/log.jsonl"],
        ["results/seed44_pop_scale/gsm8k_n2_s3_20260420_215102/log.jsonl"],
    ],
    4: [
        [
            "results/seed42_pop_scale/[1st] gsm8k_n4_20260413_143520/log.jsonl",
            "results/seed42_pop_scale/[2nd] gsm8k_n4_20260414_025348/log.jsonl",
        ],
        ["results/seed43_pop_scale/gsm8k_n4_s2/log.jsonl"],
        ["results/seed44_pop_scale/gsm8k_n4_s3_20260420_215102/log.jsonl"],
    ],
    8: [
...
COLORS = {1: "#e41a1c", 2: "#ff7f00", 4: "#4daf4a", 8: "#377eb8", 16: "#984ea3"}


def load_stitched(paths: list[str]) -> tuple[np.ndarray, np.ndarray, float | None]:
    """Load and stitch consecutive log segments, handling train_fwd resets."""
    all_fwds: list[int] = []
    all_accs: list[float] = []
    baseline = None
...
def main() -> None:
    ...
    for n, seed_runs in sorted(RUNS.items()):
        ...
        color = COLORS[n]
        valid = ~np.isnan(mean)
        ax.plot(grid[valid], mean[valid], color=color, linewidth=2, label=f"N={n}")
        ax.fill_between(
            grid[valid],
            (mean - std)[valid],
            (mean + std)[valid],
