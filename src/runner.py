...
from src.methods.vanilla_es import run_vanilla_es
from src.methods.reinforce import run_reinforce
...
def build_method(config: ExperimentConfig):
    ...
    if name in ("ars", "brs"):
        if config.method.use_ray:
            from src.methods.ars_ray import run_ars_ray
            return run_ars_ray
        return run_ars
