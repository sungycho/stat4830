...
class RunningNorm:
    ...
    def normalize(self, x):
        ...
        For dimensions with var < eps (negligible variance), return 0.0 per
        paper §4 footnote 1: avoids division by ~zero inflating the output.
        """
        x = np.asarray(x, dtype=np.float64)
        safe_std = np.where(self.var >= self.eps, np.sqrt(self.var + self.eps), np.inf)
        return np.where(np.isfinite(safe_std), (x - self.mean) / safe_std, 0.0)
