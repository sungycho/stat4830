...
@dataclass
class EnvConfig:
    name: str = "lqr"  # "lqr", "pendulum", or MuJoCo task name
    state_dim: int = 4
    action_dim: int = 2
    horizon: int = 200
    noise_std: float = 0.1
    system_seed: int = 42
    remove_survival_bonus: bool = False  # strip +1/step bonus for Hopper/Walker/Ant/Humanoid


@dataclass
class MethodConfig:
    name: str = "ars"  # "ars", "brs", "vanilla_es", or "reinforce"
    sigma: float = 0.03
    lr: float = 0.02
    N: int = 16
    b: int = 8
    ...
    variant_label: str = ""  # display label: V1, V1-t, V2, V2-t, BRS
    # Ray parallel rollouts (ARS only)
    use_ray: bool = False
    noise_table_size: int = 0  # 0 = auto-size (capped at 250 M); set explicitly to limit memory
    # REINFORCE-specific
